 ...
 * @apiGroup Recipe
 *
 * @apiParam {String} ingredients Comma-separated list of ingredient names. If no ingredients are provided, all recipes will be returned.
 * @apiParam {String} [requireAllIngredients=true] Whether to search for recipes with all or just at least one of the ingredients.
 *
 ...
 *
 * @apiError (400) InvalidIngredients Invalid ingredients.
 * @apiError (400) InvalidRequireAllIngredients Invalid requireAllIngredients value.
 * @apiError (500) {String} error Server error message.
 */
app.get('/api/getRecipes', async (req, res) => {
  let { ingredients, requireAllIngredients } = req.query;

  // Parse requireAllIngredients as a boolean
  if (requireAllIngredients === undefined || requireAllIngredients.toLowerCase() === 'true' || requireAllIngredients === '1') {
    requireAllIngredients = true;
  } else if (requireAllIngredients.toLowerCase() === 'false' || requireAllIngredients === '0') {
    requireAllIngredients = false;
  } else {
    return res.status(400).json({ error: 'Invalid requireAllIngredients value' });
  }
  ...
  if (ingredients) {
    try {
      ...
      return res.status(400).json({ error: 'Ingredients query parameter should be a comma-separated list' });
    }
  } else {
    ingredients = [];
  }
  ...
  try {
    ...
    if (ingredients.length > 0) {
      ...
      // If ingredients are provided, filter recipes by ingredients
      let result;
      if (requireAllIngredients) {
        result = await client.query(`
          ...
          FROM (
              ...
              HAVING COUNT(DISTINCT rii.ingredient_id) = $2
          ) filtered_recipes
